create_df_list_months <- function(contracts_claims_trips_data) {
  # Fonction pour calculer les variables télématiques
  compute_telematics_summaries <- function(contracts_claims_trips_data, nb_months) {
    contracts_claims_trips_data %>% 
      filter(as.numeric(date_start) <= as.numeric(contract_start_date) + 30.4375 * nb_months) %>%
      lazy_dt() %>% 
      mutate(
      ...
      summarise(
        ...
        t_avg_daily_distance     = sum(trip_distance) / (30.4375 * nb_months),
        t_avg_daily_nb_trips     = n() / (30.4375 * nb_months),
  ...
  df_ls_tele <- map(1:12, ~ compute_telematics_summaries(contracts_claims_trips_data, nb_months = .x))
